...
import { Plus, X, Loader2, GripVertical, ChevronUp, ChevronDown, CircleDashed } from "lucide-react";
import {
  Select,
  SelectContent,
  SelectItem,
...
export function RecipeForm({
  ...
  function addIngredient() {
    ...
    setIngredients((prev) => [...prev, { id, name: "", quantity: "1", unit: "pcs", isOptional: false }]);
  ...
  function updateIngredient(id: string, field: keyof Omit<IngredientRow, "id">, value: any) {
  ...
  useLayoutEffect(() => {
    if (stepsReorderTick === 0 || !stepsContainerRef.current) return;
    ...
    for (let i = 0; i < children.length; i++) {
      const el = children[i] as HTMLElement;
      const id = steps[i]?.id;
      if (id && stepsPrevPositionsRef.current.has(id)) {
        const oldPos = stepsPrevPositionsRef.current.get(id)!;
        const newPos = el.getBoundingClientRect().top;
        const delta = oldPos - newPos;
        ...
        stepsPrevPositionsRef.current.delete(id);
      }
    }
