  ...
  return (
    <div>
      ...
      <div className="flex items-start justify-between gap-4 mb-6">
        <div>
          <h1 className="font-display text-3xl text-foreground mb-2">{recipe.name}</h1>
          <div className="flex items-center gap-3 text-sm text-muted-foreground">
            <Badge className={statusColors[recipe.status]}>{t(recipe.status)}</Badge>
            {recipe.cookingTime && (
        ...
        <div className="flex gap-2 shrink-0">
          ...
          <Button variant="outline" size="sm" onClick={handleDelete} className="border-destructive text-destructive hover:bg-destructive/10">
            {t("Delete")}
          </Button>
        </div>
      </div>
      ...
      {recipe.description && (
        <p className="text-foreground mb-8 leading-relaxed max-w-prose">{recipe.description}</p>
      )}

      <section className="mb-8">
        <h2 className="text-sm font-medium text-muted-foreground mb-3">{t("Rating")}</h2>
      ...
      <section className="mb-8">
        <h2 className="text-sm font-medium text-muted-foreground mb-3">{t("Status")}</h2>
      ...
      {recipe.steps?.length > 0 && (
        <section className="mb-8">
          <h2 className="text-sm font-medium text-muted-foreground mb-3">{t("Steps")}</h2>
          <div className="flex flex-col gap-3">
            {recipe.steps.map((step: any) => (
              <div key={step.stepId} className="flex items-start gap-3">
                ...
                <p className="text-foreground leading-relaxed pt-1">{step.description}</p>
              </div>
            ))}
          </div>
        </section>
      ...
      <section>
        <h2 className="text-sm font-medium text-muted-foreground mb-3">{t("Ingredients")}</h2>
        {recipe.ingredients?.length > 0 ? (
          <div className="rounded-xl border border-border overflow-hidden">
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-muted/50 border-b border-border">
                  <th className="text-left p-3 text-muted-foreground font-normal">{t("Ingredient")}</th>
                  <th className="text-right p-3 text-muted-foreground font-normal">{t("Quantity")}</th>
                  <th className="text-right p-3 text-muted-foreground font-normal">{t("Unit")}</th>
                </tr>
              </thead>
              <tbody>
                {recipe.ingredients.map((ing: any, i: number) => (
                  <tr key={i} className="border-b border-border/50 last:border-0">
                    ...
                    <td className="p-3 text-right text-foreground">{ing.quantity}</td>
                    <td className="p-3 text-right text-muted-foreground">{unitLabels[ing.unit] ?? ing.unit}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <p className="text-muted-foreground text-sm">{t("No ingredients listed.")}</p>
        )}
      </section>
    </div>
  );
